...
// Brush size option shown as a row inside the size dropdown menu.
// Each row has a background chip (so the whole row is clickable, not just a tiny dot),
// a stroke-sample swatch that visually scales with the actual thickness, and a text label.
struct SizeOption
{
    RoundedRectangleShape rowBg;
    sf::RectangleShape swatch;
    sf::Text label;
    float thickness = 0.f;

    explicit SizeOption(sf::Font& font) : label(font, "", 14) {}

    bool isClicked(sf::Vector2i mousePos) const
    {
        return rowBg.getGlobalBounds().contains(static_cast<sf::Vector2f>(mousePos));
    }
};
...
private:
    ...
    // Update the highlight bar under the active tool
...
private:
    ...
    RoundedRectangleShape m_sizePopupPanel; // background panel behind the dropdown rows
    ...
    // Highlight bar shown under the active tool icon
    sf::RectangleShape m_toolHighlight;
};