...
{
public:
    RoundedRectangleShape(sf::Vector2f size = { 0.f, 0.f }, float radius = 10.f, unsigned int cornerPointCount = 8)
    {
        m_size = size;
        m_radius = radius;
    ...
    virtual sf::Vector2f getPoint(size_t index) const override
    {
        ...
        else
        {
            ...
            point += sf::Vector2f(cosf(angle * (float)M_PI / 180) * m_radius, sinf(angle * (float)M_PI / 180) * m_radius);
            return point;
        }
    }
    ...
    int getCornerPointCount() { return m_cornerPointCount; }
    void setCornerPointCount(unsigned int points)
    {
...
private:
    size_t m_cornerPointCount = 8;
    float m_anglePerPoint = 90.0f / 8;
